...
2. Classify the request as a main game Goal, vertical slice, bounded feature/system, engine/plugin/tool task, content/asset task, migration, diagnosis, or child Goal Packet.
...
7. Run `python scripts/lint_goal_prompt.py GOAL.md --engine detected-engine` and fix every error before emitting. Add `--main-goal` for an orchestrator Goal.
